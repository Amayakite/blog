...
<ul>
<li>
<p>熟悉使用 Slf4J、lombok、本次以模块化学习</p>
<ul>
<li>
<p>本次使用 maven 依赖如下：</p>
...
<li>
<p>然后 logback 的配置文件如下<code>logback.xml </code>固定写法</p>
...
<li>
<p>在创建线程的时候，谨记给线程取一个名字，这也是一个好的实践</p>
</li>
</ul>
<h2 id="进程和线程的基本概念" tabindex="-1"><a class="header-anchor" href="#进程和线程的基本概念" aria-hidden="true">#</a> 进程和线程的基本概念</h2>
<h3 id="进程" tabindex="-1"><a class="header-anchor" href="#进程" aria-hidden="true">#</a> 进程</h3>
<ul>
<li>程序由指令和数据组成，但这些执行要运行，数据要读写，就必须将指令加载至 CPU，数据加载至内存，在程序运行过程中还需要用到磁盘、网络等设备，进程就是用来加载指令、管理内存、管理 IO 的</li>
<li>当一个进程被运行时，从磁盘加载这个程序的代码到内存，这时就开启了一个进程</li>
<li>进程就可以视为程序的一个实例，大部分程序可以同时运行多个实例的继承（例如笔记本、画图、浏览器等），也有的程序只能启动一个实例进程（例如网易云、xx 安全卫士等）</li>
</ul>
<h3 id="线程" tabindex="-1"><a class="header-anchor" href="#线程" aria-hidden="true">#</a> 线程</h3>
<ul>
<li>一个进程之内可以分为一到多个线程</li>
<li>一个线程就是一个指令流，将指令流中的一条条指令以一定顺序交给 CPU 执行</li>
<li>Java 中，线程作为最小的调度单位，进程作为资源分配的最小单位，在 Windows 中进程是不活动的，只是作为线程的容器
<ul>
<li>Java 启动的时候有两个线程起了，分别是 main 线程和 gc 垃圾回收线程</li>
</ul>
</li>
</ul>
...
<li>进程间的通信较为复杂
<ul>
<li>同一台计算机之间的通信称为 IPC（Inter-Process-Communication）</li>
<li>不同的计算机之间线程的通讯，需要通过网络，并遵守共同的协议，例如 HTTP</li>
</ul>
</li>
<li>线程通讯相对简单，因为他们共享进程内的内存，一个例子的多线程可以访问同一个共享变量</li>
<li>线程更轻量，线程上下文切换成本一般上要比进程上下文切换低</li>
</ul>
<p>相当于他们是这样一个模型：</p>
<p><img src="/images/JavaThread/1-线程的基本介绍/1642231876301.png" alt="线程模型" loading="lazy"></p>
<h2 id="并行与并发的概念" tabindex="-1"><a class="header-anchor" href="#并行与并发的概念" aria-hidden="true">#</a> 并行与并发的概念</h2>
<p>单核 CPU 下，实际线程还是<strong>串行执行</strong>，操作系统中有一个组件叫做任务调度器，将 CPU 的时间片（Windows 下时间片最小时间大约是 15ms）分给不同的线程使用 只是由于 CPU 在线时间（时间片很短）的切换非常快，人类感觉是<em>同时运行</em>
的，总结一句话就是：<em>微观串行，宏观并行</em></p>
<p>一般会将这种线程轮循使用的 CPU 做法称之为并发，concurrent</p>
<table>
<thead>
<tr>
<th>CPU</th>
...
<p>但我们现在的 CPU 绝对不可能是单核的，所以 CPU 实际上是存在一个<strong>并行状态</strong></p>
<p><img src="/images/JavaThread/1-线程的基本介绍/1642232743901.png" alt="并行" loading="lazy"></p>
<p>引用 Rob Pike(GoLang 语言的创造者)的一段描述：</p>
<blockquote>
<p>并发(concurrent)是同一时间对(dealing with)多件事情的能力</p>
<p>并行(parallel)是同一时间手动做(doing)多件事情的能力</p>
</blockquote>
<p>生活中的例子：</p>
<ul>
<li>家庭主妇做饭，打扫卫生，给孩子喂奶，她一个人轮流交替多件事，这时就是并发</li>
<li>家庭主妇雇了个保姆，他们一起做这些事情，也有并行(这时会产生竞争，例如锅只有一口，一个人用锅的时候，另一个人就得等待)</li>
<li>雇了三个保姆，一个住专门做饭，一个专门打扫卫生，一个专门喂奶，互不干扰，这时是并行</li>
</ul>
<h2 id="线程的应用" tabindex="-1"><a class="header-anchor" href="#线程的应用" aria-hidden="true">#</a> 线程的应用</h2>
<p>从方法调用的角度来讲，如果</p>
<ul>
<li>需要等待结果返回，才能继续运行的就是同步</li>
<li>不需要等待结果返回，就能继续运行的是异步</li>
</ul>
<p>注意：同步在多线程中还有另一个意思：<strong>让多个线程步调一致</strong></p>
<blockquote>
<p>同步</p>
</blockquote>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br></div></div><h3 id="设计" tabindex="-1"><a class="header-anchor" href="#设计" aria-hidden="true">#</a> 设计</h3>
<p>多线程可以让方法变为异步的（即不要干巴巴的等待）比如说读取磁盘文件时，假设读取操作花了 5s， 如果没有线程调度机制，这五秒钟什么都干不了，只能干巴巴的等待</p>
<h3 id="结论" tabindex="-1"><a class="header-anchor" href="#结论" aria-hidden="true">#</a> 结论</h3>
<ul>
<li>比如在项目中，视频文件需要转换格式等比较费时的操作时，这时开一个新的线程处理视频转换，避免阻塞主线程</li>
<li>Tomcat 的异步 Servlet 也是类似的目的，让用户处理耗费时间较长的操作，避免阻塞 tomcat 工作线程</li>
