...
![老王](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642250739946.png)

- a，b要使用这个算盘来做一些计算，并按时间给老王支付费用
- 但是a不能一天24h使用算盘，它有时候需要休息一会儿(sleep),又或者要去上厕所(阻塞io操作)，有时候需要点一根烟，没有烟的时候思路全无(wait)，这些情况统称为**阻塞**

![a](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642250763555.png)
...
- 计算流程是这样的
- ![算盘](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642250996132.png)
- 但是由于分时系统，有一天还是发生了事故
...
- ![共享问题](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642251215715.png)
...
![自增的底层](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642252280493.png)

如果是单线程以上8行代码是顺序执行（不会交错）没有问题

![i++和i--单线程](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642252341176.png)

但多线程下这8行代码可能交错运行：

> 出现负数的情况

![多线程-负数](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642252434725.png)

> 出现正数的情况

![多线程-正数](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642252496435.png)
...
![synchronized](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642254005598.png)
...
![线程安全](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642256672659.png)
...
![a](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642258641096.png)
...
![b](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642258658516.png)
...
![monitor](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642262589004.png)

Monitor是操作系统中的对象，我们在Java中是看不到它的展示的

当我们给一个对象上锁了之后，是这样一个流程

![monitor](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642262929300.png)

这个obj中的mark word将会指向操作Monitor中的一个对象，并且如果准确无误的话，这个线程将会成为这把锁的主人，同时它的mark word将会变成如下图所示 前30位变成一个指针，指向Monitor的地址

![最终指向](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642262846464.png)
...
![new synchronized](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642263103183.png)

这个时候如果还有别的线程加入，也是同上一样，进入到Entry Set阻塞队列内进行排队等待

![其他的线程](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642263251348.png)
...
![当一个线程结束了](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642263486600.png)

到这里，也就能非常明了的知道，Monitor就是充当锁一样的角色，他就是一把锁

![流程](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642263613695.png)
...
![synchronized](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642264418106.png)
...
![故事](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642264742659.png)
...
![锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642265576228.png)

- 接着，让锁记录中的Object reference指向锁对象，并尝试用cas替换Object的Mark Word 将Mark Word的值存入锁记录

![锁2](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642265715684.png)

- 如果Cas替换成功，对象头中存储了锁记录和状态 00 ，表示由该线程给对象加锁，此时图示如下

![加锁成功](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642265829363.png)

![轻量锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642265814970.png)

那刚刚这个时候会成功，什么时候会失败呢？当然是有其他线程把最后两位替换成00了，锁地址已经指向了其他的线程，这个操作自然就失败了

![er](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642266050032.png)

当退出synchronized代码块(解锁时)，如果有取值为null的锁记录，表示有重入，这时重置锁记录，表示重入计数-1

![-1](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642266136449.png)
...
![两个轻量级锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642266554767.png)

- 此时Thread-1加轻量级锁失败，进入膨胀锁流程
    - 即为Object对象申请Monitor锁，让Object指向重量级锁地址
    - 然后自己进入Monitor的Entry List，进入阻塞状态

![重量级锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642266735253.png)

![重量级锁地址参考](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642266722639.png)
...
![自旋](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642267118338.png)

> 如果上一步中的线程最终并没有获取到锁，那么就会自旋失败，接着就是按照正常的Monitor流程进入到EntryList等待队列中阻塞

![自旋失败-阻塞](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642267235033.png)
...
![轻量级锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642308139626.png)

![偏向锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642308154612.png)
...
![对象头的几种状态](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642308454825.png)
...
![进制](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642309551672.png)

当然 你坑定会想问了，这时候我不想要它延迟，想要直接测试该怎么办

非常简单，添加个JVM参数即可

![JVM参数](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642309820122.png)

最终启动的时候，可以看到，第一个和第二个都是101

![后续测试](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642309856953.png)
...
![偏向锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642311050077.png)

原因非常简单

![偏向锁2](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642311070843.png)
...
![调用hashcode](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642311402111.png)
...
- ![小南](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642314764712.png)
- 于是老王单开了一个休息室(调用wait方法)，让小南到休息室(WaitSet)等着去了，这时锁释放开，其他人可以由老王随机安排进屋
- 直到小M将烟送来了，大叫一声：【你的烟到了】(调用notify方法)
- ![烟](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642314863682.png)
- 小南此时可以离开休息室，重新进入竞争锁队列
- ![小南2](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642314931554.png)

### 原理之wait和notify

![wait和notify](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642315003952.png)
...
![guarded](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642318697632.png)
...
![结果](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642319757511.png)
...
![unpark](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642327251726.png)

结果是：成功解锁，接下来的代码畅通无阻

![over](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642327276001.png)
...
![park1](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642328218880.png)

![park2](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642328237311.png)

## 重新理解线程的状态转换

![线程的状态](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642330625727.png)
...
![死锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642333253102.png)
...
![检测死锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642333385784.png)

如果检测到了，就会出现如下所示的内容

![检测到了死锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642333410535.png)

### 死锁问题-哲学家就餐

![哲学家就餐](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642334061689.png)
...
![死锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642334359828.png)

当然，你可能run的比我的快一些，接下来开`jconsole`看看

可以发现，他们全都被锁住了...

![死锁的体现](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642334479168.png)
...
![获取锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335108435.png)

顺序加锁的解决方案

![解锁](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335165021.png)

通过给定优先级或者设定时间，避免死锁

我们来改进下之前的死锁代码

![1](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335278286.png)

可以看到一直在run了

![结果](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335365891.png)

并且没有检测到死锁

![死锁null](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335378993.png)

但是你能发现一个情况，程序在运行了一段时间之后，就很难见到阿基米德

![1](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642335420560.png)
...
![运行结果](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642336433462.png)
...
![a](http://81.68.162.137:9300/blog-notes/images/JavaThread/3-共享模型之管程/1642337150360.png)
